...
void loop() {
  ...
  if(isMessage){
    ...
    if(request.indexOf("IL")!=-1 || request.indexOf("CONNECTED") != -1 ||
  ...
  if (Serial1.available() > 0) {
    ...
    else if(command.indexOf("ECTED") !=-1 || command.indexOf("GOT") != -1 ||command.indexOf("FAIL") != -1){
       WifiConnect = true;
       isMessage = true;
       delay(300);
       request = command;
    ...
    else if(command.indexOf("GOT") != -1 || command.indexOf("IP") != -1 || alreadyConnect){
      WifiConnect = true;
    }
